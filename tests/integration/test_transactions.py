...
async def wallet_headers(client: AsyncClient, register_user, db_session: AsyncSession) -> tuple[dict, Wallet]:
    """Register a user, create their wallet via the API, and return (auth headers, wallet)."""
    headers = await register_user("transactions@example.com")
    resp = await client.post("/wallet", headers=headers)
    wallet = await db_session.get(Wallet, uuid.UUID(resp.json()["id"]))
    return headers, wallet
...
class TestListTransactionsFilters:
    ...
    async def test_end_date_is_inclusive_of_the_whole_day(self, client: AsyncClient, wallet_headers, make_transaction):
        ...
        resp = await client.get("/transactions", params={"end_date": now.strftime("%d/%m/%Y")}, headers=headers)
    ...
    async def test_rejects_invalid_date_format(self, client: AsyncClient, wallet_headers):
        ...
        headers, _wallet = wallet_headers

        resp = await client.get(
            "/transactions", params={"start_date": "2026-07-17"}, headers=headers
        )
    ...
    async def test_filters_by_min_and_max_amount(self, client: AsyncClient, wallet_headers, make_transaction):
        ...
        resp = await client.get(
            "/transactions", params={"min_amount": "10.00", "max_amount": "30.00"}, headers=headers
        )
    ...
    async def test_filters_by_type(self, client: AsyncClient, wallet_headers, make_transaction):
        ...
        resp = await client.get("/transactions", params={"type": "WITHDRAWAL"}, headers=headers)
    ...
    async def test_filters_by_status(self, client: AsyncClient, wallet_headers, make_transaction):
        ...
        resp = await client.get("/transactions", params={"status": "PENDING"}, headers=headers)
    ...
    async def test_paginates_with_page_and_page_size(self, client: AsyncClient, wallet_headers, make_transaction):
        ...
        resp = await client.get("/transactions", params={"page": 2, "page_size": 2}, headers=headers)
    ...
    async def test_only_returns_transactions_for_the_authenticated_users_wallet(
        ...
        other_headers = await register_user("other.transactions@example.com")
        other_wallet_resp = await client.post("/wallet", headers=other_headers)
